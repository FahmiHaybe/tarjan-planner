...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            choice = validate_input("Choose an option (1-4): ", r"^[1-4]$","Invalid option. Please enter a number between 1 and 4.")

            if choice == '1':
                input_mode = validate_input("Enter the weight for the graph edges (time or cost): ", r"^(time|cost)$","Invalid weight. Please enter 'time' or 'cost'.")
                main(input_mode, relatives_manager.relatives, relatives_manager.home, transport_manager.transports)
            elif choice == '2':
                manage_entities(relatives_manager, "relative", Relative,["name", "street_name", "district", "lat", "lng"])
            elif choice == '3':
                manage_entities(transport_manager, "transport mode", Transport,["name", "speed_kmh", "transfer_time_min", "cost_per_km"])
            elif choice == '4':
